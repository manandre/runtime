...
namespace System.Threading.Tasks.Dataflow.Tests
{
    public class DataflowBlockExtensionsTests
    {
        ...
        {
            Assert.Throws<ArgumentNullException>(() => ((ISourceBlock<int>)null).ReceiveAllAsync());
            Assert.Throws<ArgumentNullException>(() => ((ISourceBlock<int>)null).ReceiveAllAsync(new CancellationToken(true)));
        }
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task ReceiveAllAsync_UseMoveNextAsyncAfterCompleted_ReturnsFalse(bool completeWhilePending)
        {
            var source = new BufferBlock<int>();
