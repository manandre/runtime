...
namespace System.IO.Compression
{
    ...
    public sealed partial class ZstandardStream : Stream
    {
        ...
        /// <exception cref="InvalidOperationException">Cannot perform read operations on a <see cref="ZstandardStream" /> constructed with <see cref="CompressionMode.Compress" />.
        /// -or-
        ...
        /// <summary>Reads a sequence of bytes from the current Zstandard stream to a byte span and advances the position within the Zstandard stream by the number of bytes read.</summary>
        ...
        /// <remarks>Use the <see cref="System.IO.Compression.ZstandardStream.CanRead" /> property to determine whether the current instance supports reading. Use the <see langword="System.IO.Compression.ZstandardStream.ReadAsync" /> method to read asynchronously from the current stream.
        /// This method read a maximum of `buffer.Length` bytes from the current stream and store them in <paramref name="buffer" />. The current position within the Zstandard stream is advanced by the number of bytes read; however, if an exception occurs, the current position within the Zstandard stream remains unchanged. This method will block until at least one byte of data can be read, in the event that no data is available. `Read` returns 0 only when there is no more data in the stream and no more is expected (such as a closed socket or end of file). The method is free to return fewer bytes than requested even if the end of the stream has not been reached.
        /// Use <see cref="System.IO.BinaryReader" /> for reading primitive data types.</remarks>
        public override int Read(Span<byte> buffer)
        {
            if (_mode != CompressionMode.Decompress)
                throw new InvalidOperationException(SR.ZstandardStream_Compress_UnsupportedOperation);
            EnsureNotDisposed();
        ...
        /// <exception cref="System.NotSupportedException">The current <see cref="System.IO.Compression.ZstandardStream" /> implementation does not support the read operation.</exception>
        ...
        /// <returns>The number of bytes read from the stream, between 0 (zero) and the number of bytes you requested. <see cref="System.IO.Compression.ZstandardStream" /> returns 0 only at the end of the stream; otherwise, it blocks until at least one byte is available.</returns>
        /// <exception cref="System.ArgumentNullException"><paramref name="asyncResult" /> is <see langword="null" />.</exception>
        /// <exception cref="System.ArgumentException"><paramref name="asyncResult" /> did not originate from a <see cref="System.IO.Compression.ZstandardStream.BeginRead(byte[],int,int,System.AsyncCallback,object)" /> method on the current stream.</exception>
        ...
        /// <summary>Asynchronously reads a sequence of bytes from the current Zstandard stream, writes them to a byte array starting at a specified index, advances the position within the Zstandard stream by the number of bytes read, and monitors cancellation requests.</summary>
        /// <param name="buffer">The buffer to write the data into.</param>
        /// <param name="offset">The byte offset in <paramref name="buffer" /> at which to begin writing data from the Zstandard stream.</param>
        /// <param name="count">The maximum number of bytes to read.</param>
        /// <param name="cancellationToken">The token to monitor for cancellation requests. The default value is <see cref="System.Threading.CancellationToken.None" />.</param>
        /// <returns>A task that represents the asynchronous read operation, which wraps the total number of bytes read into the <paramref name="buffer" />. The result value can be less than the number of bytes requested if the number of bytes currently available is less than the requested number, or it can be 0 (zero) if the end of the Zstandard stream has been reached.</returns>
        ...
        /// Use the <see cref="System.IO.Compression.ZstandardStream.CanRead" /> property to determine whether the current instance supports reading.
        ...
        /// <summary>Asynchronously reads a sequence of bytes from the current Zstandard stream, writes them to a byte memory range, advances the position within the Zstandard stream by the number of bytes read, and monitors cancellation requests.</summary>
        /// <param name="buffer">The region of memory to write the data into.</param>
        /// <param name="cancellationToken">The token to monitor for cancellation requests. The default value is <see cref="System.Threading.CancellationToken.None" />.</param>
        /// <returns>A task that represents the asynchronous read operation, which wraps the total number of bytes read into the buffer. The result value can be less than the number of bytes allocated in the buffer if that many bytes are not currently available, or it can be 0 (zero) if the end of the Zstandard stream has been reached.</returns>
        /// <remarks>The `ReadAsync` method enables you to perform resource-intensive I/O operations without blocking the main thread. This performance consideration is particularly important in a Windows 8.x Store app or desktop app where a time-consuming stream operation can block the UI thread and make your app appear as if it is not working. The async methods are used in conjunction with the <see langword="async" /> and <see langword="await" /> keywords in Visual Basic and C#.
        /// Use the <see cref="System.IO.Compression.ZstandardStream.CanRead" /> property to determine whether the current instance supports reading.
        /// If the operation is canceled before it completes, the returned task contains the <see cref="System.Threading.Tasks.TaskStatus.Canceled" /> value for the <see cref="System.Threading.Tasks.Task.Status" /> property.</remarks>
