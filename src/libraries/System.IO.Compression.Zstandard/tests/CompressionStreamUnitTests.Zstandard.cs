...
namespace System.IO.Compression
{
    public class ZstandardStreamUnitTests : CompressionStreamUnitTestBase
    {
        ...
        [Fact]
        public void InvalidCompressionLevel()
        {
            Assert.Throws<ArgumentOutOfRangeException>("compressionLevel", () => new ZstandardEncoder(42));
            Assert.Throws<ArgumentOutOfRangeException>("compressionLevel", () => new ZstandardEncoder(-1_000_000));
            Assert.Throws<ArgumentOutOfRangeException>("compressionLevel", () => ZstandardEncoder.TryCompress(new ReadOnlySpan<byte>(), new Span<byte>(), out int bytesWritten, 42));
            Assert.Throws<ArgumentOutOfRangeException>("compressionLevel", () => ZstandardEncoder.TryCompress(new ReadOnlySpan<byte>(), new Span<byte>(), out int bytesWritten, -1_000_000));
        }

        [Fact]
        public void GetMaxCompressedSize_Basic()
        {
            Assert.Throws<ArgumentOutOfRangeException>("inputSize", () => ZstandardEncoder.GetMaxCompressedLength(-1));
            Assert.InRange(ZstandardEncoder.GetMaxCompressedLength(int.MaxValue/2), 0, int.MaxValue);
            Assert.Equal(64, ZstandardEncoder.GetMaxCompressedLength(0));
        }
        ...
        public void GetMaxCompressedSize()
        {
            ...
            int maxCompressedSize = ZstandardEncoder.GetMaxCompressedLength((int)new FileInfo(uncompressedFile).Length);
        ...
        public void Decompress_WithEmptyDestination()
        {
            ...
            Assert.False(ZstandardDecoder.TryDecompress(source, destination, out int bytesWritten), "TryDecompress completed successfully but should have failed due to too short of a destination array");
            Assert.Equal(0, bytesWritten);

            ZstandardDecoder decoder = default;
        ...
        public void Decompress_WithEmptySource()
        {
            ...
            Assert.False(ZstandardDecoder.TryDecompress(source, destination, out int bytesWritten), "TryDecompress completed successfully but should have failed due to too short of a source array");
            Assert.Equal(0, bytesWritten);
        ...
        public void Compress_WithEmptyDestination()
        {
            ...
            Assert.False(ZstandardEncoder.TryCompress(source, destination, out int bytesWritten), "TryCompress completed successfully but should have failed due to too short of a destination array");
            Assert.Equal(0, bytesWritten);

            ZstandardEncoder encoder = default;
        ...
        public void Compress_WithEmptySource()
        {
            ...
            Assert.True(ZstandardEncoder.TryCompress(source, destination, out int bytesWritten));
        ...
        /// <summary>
        /// Test that the decoder can handle partial chunks of flushed encoded data sent from the ZstandardEncoder
        /// </summary>
        [Fact]
        public void RoundTrip_Chunks()
        {
            ...
            for (int i = 0; i < totalSize; i += chunkSize)
            {
                ...
                byte[] compressed = new byte[ZstandardEncoder.GetMaxCompressedLength(chunkSize)];
        ...
        public void ReadFully(string testFile)
        {
            ...
            Assert.True(ZstandardDecoder.TryDecompress(source, destination, out int bytesWritten), "TryDecompress did not complete successfully");
        ...
        {
            byte[] correctUncompressedBytes = File.ReadAllBytes(testFile);
            byte[] compressedBytes = new byte[ZstandardEncoder.GetMaxCompressedLength(correctUncompressedBytes.Length)];
            byte[] actualUncompressedBytes = new byte[ZstandardEncoder.GetMaxCompressedLength(correctUncompressedBytes.Length)];

            Span<byte> destination = new Span<byte>(compressedBytes);

            Assert.True(ZstandardEncoder.TryCompress(correctUncompressedBytes, destination, out int bytesWritten));
            Assert.True(ZstandardDecoder.TryDecompress(destination.Slice(0,bytesWritten), actualUncompressedBytes, out bytesWritten));
            Assert.Equal(correctUncompressedBytes.Length, bytesWritten);
        ...
        public static void Roundtrip_WriteByte_ReadByte_Success(int totalLength, CompressionLevel level)
        {
            ...
            {
                var bs = new ZstandardStream(ms, level);
            ...
            using (var bs = new ZstandardStream(ms, CompressionMode.Decompress))
            {
        ...
        {
            ZstandardEncoder.TryCompress(input, output, out int bytesWritten);
        }

        private static void Decompress_WithoutState(ReadOnlySpan<byte> input, Span<byte> output)
        {
            ZstandardDecoder.TryDecompress(input, output, out int bytesWritten);
        }
        ...
        private static MemoryStream Compress_Stream(ReadOnlySpan<byte> input, CompressionLevel compressionLevel)
        {
            using (var inputStream = new MemoryStream(input.ToArray()))
            {
                ...
                var compressor = new ZstandardStream(outputStream, compressionLevel, true);
        ...
        private static MemoryStream Decompress_Stream(ReadOnlySpan<byte> input)
        {
            ...
            {
                var outputStream = new MemoryStream();
                var decompressor = new ZstandardStream(inputStream, CompressionMode.Decompress, true);
