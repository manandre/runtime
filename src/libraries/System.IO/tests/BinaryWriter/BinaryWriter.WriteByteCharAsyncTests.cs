...
{
    public class BinaryWriter_WriteByteCharAsyncTests
    {
        ...
        public async Task BinaryWriter_WriteCharAsyncTest_Negative()
        {
            ...
            await using BinaryWriter writer = new BinaryWriter(mem, Encoding.Unicode);
            // between 55296 <= x < 56319
            int[] randomNumbers = new int[] { 55296, 56318, 55305, 56019, 55888, 55900, 56251 };
            for (int i = 0; i < randomNumbers.Length; i++)
            {
                char ch = (char)randomNumbers[i];
                await AssertExtensions.ThrowsAsync<ArgumentException>(null, () => writer.WriteAsync(ch).AsTask());
            }
        ...
        public async Task BinaryWriter_WriteCharTest2()
        {
            ...
            await using BinaryWriter writer = new BinaryWriter(stream, Encoding.GetEncoding(codepageName));

            byte[] bytes = { 0x01, 0x02, 0x03, 0x04, 0x05 };

            await writer.WriteAsync((char)0x30ca);
            await writer.WriteAsync(bytes);
            await writer.FlushAsync();
            await writer.WriteAsync('\0');

            stream.Seek(0, SeekOrigin.Begin);
            using BinaryReader reader = new BinaryReader(stream, Encoding.GetEncoding(codepageName));
        ...
        public async Task BinaryWriter_WriteByteAsyncTest()
        {
            ...
            for (ii = 0; ii < bytArr.Length; ii++)
                await dw2.WriteAsync(bytArr[ii]);
        ...
        public async Task BinaryWriter_WriteBArrayAsyncTest_Negative()
        {
            ...
            await Assert.ThrowsAsync<ArgumentNullException>(() => dw2.WriteAsync((byte[])null).AsTask());
            mstr.Dispose();
            await dw2.DisposeAsync();
            ...
            await Assert.ThrowsAsync<ArgumentNullException>(() => dw2.WriteAsync((byte[])null, 0, 0).AsTask());
            ...
            for (int iLoop = 0; iLoop < iArrInvalidValues.Length; iLoop++)
            {
                ...
                await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() => dw2.WriteAsync(bArr, iArrInvalidValues[iLoop], 0).AsTask());
                // [] ArgumentOutOfRangeException for negative count
                await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() => dw2.WriteAsync(bArr, 0, iArrInvalidValues[iLoop]).AsTask());
            }
            await dw2.DisposeAsync();
            mstr.Dispose();
            ...
            {
                // [] Offset out of range
                await AssertExtensions.ThrowsAsync<ArgumentException>(null, () => dw2.WriteAsync(bArr, iArrLargeValues[iLoop], 0).AsTask());
                // [] Invalid count value
                await AssertExtensions.ThrowsAsync<ArgumentException>(null, () => dw2.WriteAsync(bArr, 0, iArrLargeValues[iLoop]).AsTask());
            }
            await dw2.DisposeAsync();
            mstr.Dispose();
        }
        ...
        public async Task BinaryWriter_WriteBArrayTest2()
        {
            ...
            await dw2.WriteAsync(bArr, 0, bArr.Length);
            await dw2.FlushAsync();
            mstr.Position = 0;

            using BinaryReader dr2 = new BinaryReader(mstr);
            byte[]bReadArr = new byte[bArr.Length];
            int ReturnValue = dr2.Read(bReadArr, 0, bArr.Length);
        ...
        public async Task BinaryWriter_WriteCharArrayAsyncTest_Negative()
        {
            ...
            Stream mstr = CreateStream();
            BinaryWriter dw2 = new BinaryWriter(mstr);
            await Assert.ThrowsAsync<ArgumentNullException>(() => dw2.WriteAsync((char[])null).AsTask());
            await dw2.DisposeAsync();
            mstr.Dispose();
            ...
            mstr = CreateStream();
            dw2 = new BinaryWriter(mstr);
            await Assert.ThrowsAsync<ArgumentNullException>(() => dw2.WriteAsync((char[])null, 0, 0).AsTask());
            ...
            {
                // [] ArgumentOutOfRange for negative offset
                await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() => dw2.WriteAsync(chArr, iArrInvalidValues[iLoop], 0).AsTask());
                // [] negative count.
                await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() => dw2.WriteAsync(chArr, 0, iArrInvalidValues[iLoop]).AsTask());
            }
            mstr.Dispose();
            await dw2.DisposeAsync();
            ...
            {
                // [] Offset out of range
                await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() => dw2.WriteAsync(chArr, iArrLargeValues[iLoop], 0).AsTask());
                // [] Invalid count value
                await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() => dw2.WriteAsync(chArr, 0, iArrLargeValues[iLoop]).AsTask());
            }
            mstr.Dispose();
            await dw2.DisposeAsync();
        }
        ...
        public async Task BinaryWriter_WriteCharArrayTest2()
        {
            ...
            await writer.WriteAsync(randomChars);
            mem.Position = 0;
            using BinaryReader reader = new BinaryReader(mem, Encoding.Unicode);
        ...
        public async Task BinaryWriter_WriteCharArrayTest3()
        {
            ...
            await dw2.WriteAsync(chArr, 0, chArr.Length);
            await dw2.FlushAsync();
            mstr.Position = 0;
        ...
        public async Task BinaryWriter_WriteSpan()
        {
            ...
            Memory<byte> byteSpan = new Memory<byte>(bytes);
            Memory<char> charSpan = new Memory<char>(chars);

            using Stream memoryStream = CreateStream();
            await using BinaryWriter binaryWriter = new BinaryWriter(memoryStream, Encoding.Unicode);

            await binaryWriter.WriteAsync(byteSpan);
            await binaryWriter.WriteAsync(charSpan);
