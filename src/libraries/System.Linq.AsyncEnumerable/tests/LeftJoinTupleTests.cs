...
{
    public class LeftJoinTupleTests : AsyncEnumerableTests
    {
        [Fact]
        ...
        public async Task VariousValues_MatchesEnumerable_String()
        {
            ...
            foreach (int length in new[] { 0, 1, 2, 1000 })
            {
                ...
                {
                    await AssertEqual(
                        values.LeftJoin(values, s => s.Length > 0 ? s[0] : ' ', s => s.Length > 1 ? s[1] : ' '),
                        source.LeftJoin(source, s => s.Length > 0 ? s[0] : ' ', s => s.Length > 1 ? s[1] : ' '));

                    await AssertEqual(
                        values.LeftJoin(values, s => s.Length > 0 ? s[0] : ' ', s => s.Length > 1 ? s[1] : ' '),
                        source.LeftJoin(source, async (s, ct) => s.Length > 0 ? s[0] : ' ', async (s, ct) => s.Length > 1 ? s[1] : ' '));
                }
            }
        }
#endif
